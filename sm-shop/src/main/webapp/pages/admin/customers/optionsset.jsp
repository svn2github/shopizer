					...
					<jsp:include page="/common/adminTabs.jsp" />
  					 ...
  					 <div class="tab-content">
    					...
    					<div class="tab-pane active" id="customer-section">
								...
								<h3>
									<s:message code="menu.customer-options-set" text="Manage customer options set" />
								</h3>	

								<c:url var="createOptionSet" value="/admin/customers/optionsset/save.html"/> 
		                  		<form:form method="POST" commandName="optionSet" action="${createOptionSet}" cssClass="form-inline">
				
		                  			 		
		                        					<label><s:message code="label.customer.option" text="Option"/></label>
		                        			
		                        					<form:select path="pk.customerOption">
						  								<form:options items="${customerOptions}" itemValue="id" itemLabel="descriptionsSettoList[0].name"/>
					       							</form:select>
		                        			
		                 			 
		                  			 
		                        					<label><s:message code="label.customer.option.value" text="Option value"/></label>
		                        					
		                        					<form:select path="pk.customerOptionValue">
						  								<form:options items="${customerOptionsValues}" itemValue="id" itemLabel="descriptionsSettoList[0].name"/>
					       							</form:select>
								 ...
								 <c:set value="/admin/customers/optionsset/paging.html" var="pagingUrl" scope="request"/>
								 <c:set value="/admin/customers/optionsset/remove.html" var="removeUrl" scope="request"/>
